    ...
    if re.search(instagram_regex, text):
        ...
        if vid:
            try:
                ...
                await bot.send_document(message.chat.id, vid, caption="Downloaded via @UltimateSaverBot")
                ...
                with open(f"{message.message_id}.mp4", 'wb') as video:
                    ...
                    video.write(rrr.content)
                input_file = types.InputFile(f"{message.message_id}.mp4")
                await bot.send_document(message.chat.id, document=input_file,caption="Downloaded via @UltimateSaverBot")
                shazammusic = await shazamtop(f"{message.message_id}.mp4")
                title = shazammusic['title']
        ...
        await bot.send_video(message.chat.id, video=input_file,caption="Downloaded via @UltimateSaverBot")
        os.remove(f"{message.message_id}.mp4")

    elif any(substring in text for substring in ["youtube.com"]):
        # msg_del = await message.reply("⏳")
        msg_del = await bot.send_chat_action(message.chat.id, types.ChatActions.TYPING)

        try:
            vid = await get_download_url(text)
            await bot.send_video(chat_id=message.chat.id, video=vid, caption="Downloaded via @UltimateSaverBot")
        except Exception as err:
